...
```rust
use crate::db::models::{Subscription, NewSubscription};
...
pub async fn add_subscription(
    ...
    mqtt_manager: State<'_, Arc<Mutex<MqttManager>>>,
    server_id: i64,
    topic: String,
    qos: i32,
    ...
    let subscription = {
        ...
        db.insert_subscription(&new_sub).map_err(|e| e.to_string())?
    };
    
    // 如果已连接，则订阅主题
    {
        let manager = mqtt_manager.lock().await;
        if manager.is_connected(&server_id.to_string()) {
            manager.subscribe(&server_id.to_string(), &topic, qos as u8)
                .await
...
pub async fn remove_subscription(
    ...
    // 如果已连接，则取消订阅
    {
        let manager = mqtt_manager.lock().await;
        if manager.is_connected(&server_id.to_string()) {
            manager.unsubscribe(&server_id.to_string(), &topic)
                .await
                .map_err(|e| e.to_string())?;
        }
    }
    ...
    // 从数据库删除
    let db = db.lock().await;
    db.delete_subscription(subscription_id).map_err(|e| e.to_string())?;
...
pub async fn get_subscriptions(
    ...
    let db = db.lock().await;
    db.get_subscriptions_by_server(server_id).map_err(|e| e.to_string())
}
...
```rust
use crate::db::models::{MessageHistory, NewMessageHistory};
use crate::db::Database;
use crate::mqtt::MqttManager;
use tauri::State;
use std::sync::Arc;
...
pub async fn publish_message(
    ...
    let payload_bytes = match message.format.as_str() {
        "hex" => hex::decode(&message.payload)
    ...
    let history = {
        ...
        db.insert_message_history(&new_history).map_err(|e| e.to_string())?
    };
...
pub async fn clear_message_history(
    ...
    let db = db.lock().await;
    db.clear_message_history(server_id).map_err(|e| e.to_string())
}
```
...
impl Database {
    ...
    pub fn insert_subscription(&self, sub: &NewSubscription) -> Result<Subscription> {
        self.conn.execute(
            ...
            params![sub.server_id, sub.topic, sub.qos, sub.is_active],
        )?;
    ...
    pub fn delete_subscription(&self, id: i64) -> Result<()> {
        self.conn.execute("DELETE FROM subscriptions WHERE id = ?1", params![id])?;
        Ok(())
    }
    ...
    pub fn insert_message_history(&self, msg: &NewMessageHistory) -> Result<MessageHistory> {
        self.conn.execute(
            ...
            params![msg.server_id, msg.topic, msg.payload, msg.payload_format, msg.direction, msg.qos, msg.retain],
        )?;
    ...
    pub fn get_message_history(&self, server_id: i64, limit: i64, offset: i64) -> Result<Vec<MessageHistory>> {
        let mut stmt = self.conn.prepare(
    ...
    pub fn clear_message_history(&self, server_id: i64) -> Result<()> {
        self.conn.execute("DELETE FROM message_history WHERE server_id = ?1", params![server_id])?;
        Ok(())
    }
}
```
